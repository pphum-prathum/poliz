...
void main(){
  ...
  Future<void> addIncident(WidgetTester tester) async {
  ...
  testWidgets('TC_INCIDENT_SEARCH_01', (WidgetTester tester) async {
    await loginAndGoToIncidentImportantRanking(tester);
    await addIncident(tester);
  ...
  testWidgets('TC_INCIDENT_SEARCH_02', (WidgetTester tester) async {
    ...
    await tester.enterText(find.byKey(const ValueKey('searchField')), 'Traffic Accident');
    await tester.pumpAndSettle();

    expect(find.text('Loaded: 0 incidents'), findsOne);
  });
}